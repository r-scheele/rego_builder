...
async def write_policy(
    ...
    if provider == "gitlab":
        rego_rule.repo_url = GitLabOperations(rego_rule.repo_id, dependencies["token"]).repo_url_from_id()
...
async def modify_policy(
    ...
    WriteRego(
        dependencies["token"], updated_policy["repo_url"], dependencies["login"], provider
    ).write_to_file(policies)
...
async def remove_policy(
    ...
    if provider == "gitlab":
        WriteRego(dependencies["token"], repo_id, user, provider).write_to_file(policies)
    WriteRego(dependencies["token"], repo_url, user, provider).write_to_file(policies)
